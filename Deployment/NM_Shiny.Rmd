...
```{r}
HTML(paste("In this dashboard, you will find information on how New Mexico is affected by changes in the interpretation of the Clean Water Act's scope. These changes means that most of New Mexico's streams and rivers will no longer be protected. Some places and public water systems (PWS) will be more affected than others. This dashboard shows both the places and the socio-economic characteristics of each of these places using the Human Development Index (HDI) and Drinking Water Vulnerability Index.", "The HDI is a statistic used by the United Nations to rank countries into tiers of human development. It looks at life expectancy, education, and per capita income. This has been modified to for use in New Mexico where it is to rank and evaluate each county in terms of its human development.", "The Drinking Water Vulnerability Index measures for New Mexican public water systems, the degree of exposure of surface water sources and the community served by the PWS's ability to circumvent a potential contamination or negative impact to their drinking water. The index is on a scale of 1 to 10, with 10 being the most vulnerable. The index takes into consideration household income, degree of impact to the PWS surface water intakes, and the availability of alternative sources of water.", sep = "<br/>"))
```
...
### Vulnerability Map
```{r}
# nm_pws_merged_complete_cases <- nm_pws_merged[complete.cases(nm_pws_merged$Index),]
...
renderLeaflet({
  vul_pal <- colorBin("Reds", domain = nm_pws_merged$Index, bins = c(2,4,6,8))
    leaflet() %>%
      ...
      addCircleMarkers(data = nm_surface_water_intake, label = ~SYSTEM_NAM, group = "Public Water Sources",
                       color = "#525252", radius = 4, stroke = F, fillOpacity = 0.7, clusterOptions = markerClusterOptions()) %>%
      addLayersControl(overlayGroups = c("Water System", "Public Water Sources"),
                       options = layersControlOptions(collapsed = FALSE)) %>%
      hideGroup("Public Water Sources") %>%
      addLegend("bottomright", pal = vul_pal, values = nm_pws_merged$Index, title = "Vulnerability Index", opacity = 0.7)
    })
```
