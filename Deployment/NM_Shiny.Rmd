...
renderLeaflet({
  vul_pal <- colorBin("Reds", domain = nm_pws_merged_complete_cases$Index, bins = c(2,4,6,8))
    leaflet() %>%
      addProviderTiles(providers$Stamen.Toner, options = providerTileOptions(zIndex = 7)) %>%
      setView(lng = -105.87, lat = 34.52, zoom = 7) %>%
      # addPolygons(data = nm_counties, group = "County",  
      #             fillOpacity = 0, weight = 1.5, color = "white") %>%
      addPolygons(data = nm_pws_merged_complete_cases, label = nm_pws_merged_complete_cases$PublicSyst.y, group = "Water System",
      ...
      addLegend("bottomright", pal = vul_pal, values = nm_pws_merged_complete_cases$Index, title = "Vulnerability Index", opacity = 0.7)
    })
```
...
```{r}
renderPlot({
  ggplot(nm_pws_merged_complete_cases, aes(x = Population, y = as.numeric(MedianIncome))) + 
    geom_point(aes(color = Index)) +
    scale_color_gradient(low = "#fee0d2", high = "#de2d26") + 
    geom_text_repel(aes(label = PublicSyst.y), size = 2.0) + 
...
reactive_county <- reactive({
    ...
    # vul_pal <- colorBin("Reds", domain = nm_pws_merged_complete_cases$Index, bins = c(2,4,6,8))
    # nm_pws_county_filtered <- nm_pws_merged_complete_cases %>%
    #   filter(County == str_to_upper(input$county))
    ...
    leaflet() %>%
      ...
      addPolygons(data = nm_pws_merged_complete_cases, label = paste(nm_pws_merged_complete_cases$PublicSyst.y,
                                                                     nm_pws_merged_complete_cases$Index, sep = ": "), 
