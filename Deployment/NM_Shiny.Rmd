...
renderLeaflet({
    ...
    leaflet() %>%
      ...
      hideGroup(c("Unprotected Streams", "Protected Streams", "Pollutant Sources", "County")) %>%
      # setMaxBounds(lng1 = -108, lng2 = -103, lat1 = 32, lat2 = 36) %>%
      addLegend("bottomright", pal = pal, values = nm_census_tracts$HDI, title = "HDI", opacity = 0.7)
   })
```
...
```{r}
renderPlot({
  ggplot(nm_streams_summary_df, aes(x = `Domestic Self Supplied Pct`, y = pct_unprotected)) + 
    geom_point() + 
    geom_text_repel(aes(label = NAMELSAD), size = 2.0) + 
...
reactive_county <- reactive({
    ...
    leaflet() %>%
      ...
      addLayersControl(overlayGroups = c("County", "HDI", "Unprotected Streams", "Protected Streams"),
                       options = layersControlOptions(collapsed = FALSE)) %>%
      hideGroup(c("Unprotected Streams", "Protected Streams", "County")) %>%
